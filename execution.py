...
class Execution:
    ...
    def execute_market_sell(self, symbol: str, amount: float) -> Optional[Dict]:
        if self.paper:
            logger.info(f"[PAPER] Executed MARKET SELL for {amount} {symbol}")
            return {'status': 'closed', 'symbol': symbol, 'amount': amount, 'side': 'sell', 'type': 'market', 'price': None}

        try:
            logger.info(f"[LIVE] Executing MARKET SELL for {amount} {symbol}")
    ...
    def execute_limit_buy(self, symbol: str, amount: float, price: float) -> Optional[Dict]:
        if self.paper:
            logger.info(f"[PAPER] Placed LIMIT BUY (Take Profit) for {amount} {symbol} at {price:.2f}")
            return {'status': 'open', 'symbol': symbol, 'amount': amount, 'side': 'buy', 'type': 'limit', 'price': price}

        try:
            logger.info(f"[LIVE] Placing LIMIT BUY for {amount} {symbol} at {price:.2f}")
            order = self.exchange.create_limit_buy_order(symbol, amount, price)
            return order
        except Exception as e:
            logger.error(f"Failed to execute limit buy (TP) for {symbol}: {e}")
            return None

    def execute_stop_market_buy(self, symbol: str, amount: float, stop_price: float) -> Optional[Dict]:
        if self.paper:
            logger.info(f"[PAPER] Placed STOP MARKET BUY (Stop Loss) for {amount} {symbol} at {stop_price:.2f}")
            return {'status': 'open', 'symbol': symbol, 'amount': amount, 'side': 'buy', 'type': 'stop', 'stopPrice': stop_price}

        try:
            logger.info(f"[LIVE] Placing STOP MARKET BUY for {amount} {symbol} at {stop_price:.2f}")
            params = {'stopPrice': stop_price}
            order = self.exchange.create_order(symbol, 'market', 'buy', amount, None, params)
            return order
        except Exception as e:
            logger.error(f"Failed to execute stop buy (SL) for {symbol}: {e}")
            return None
